...
student_names = load_workbook("csattendance.xlsx") #Can change excel sheet extracted from MSForms
...
for rows in range(2, number_of_names+2):
        final_attendance.append(names_list_unique[rows-2])
        updated_attendance["A" + str(rows)] = names_list_unique[rows-2]
        updated_attendance["B" + str(rows)] = names_list.count(names_list_unique[rows-2])
        if (names_list.count(names_list_unique[rows-2]) >= voting_eligibility):
            updated_attendance["C"+str(rows)] = True
        else:
            updated_attendance["C"+str(rows)] = False
        if (names_list.count(names_list_unique[rows-2]) >= running_eligibility):
